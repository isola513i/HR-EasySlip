...
export function otSubmittedHtml(p: Params): string {
  return `<!DOCTYPE html>
<html lang="th">
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:480px;">
  ...
  <tr><td style="background:${BRAND.card};border:1px solid ${BRAND.border};border-radius:12px;padding:28px;">
    <p style="margin:0 0 8px;font-size:16px;font-weight:600;color:${BRAND.dark};">มีคำขอ OT ใหม่ / New Overtime Request</p>
    <p style="margin:0 0 20px;font-size:14px;color:${BRAND.muted};">
      ${p.employeeName} (${p.employeeCode}) ยื่นคำขอทำงานล่วงเวลา
    </p>
...
export function otSubmittedText(p: Params): string {
  return [
    `${BRAND.name} — มีคำขอ OT ใหม่ / New Overtime Request`,
    "",
