...
export function HROverview() {
  ...
  if (error) {
    return (
      <div className="py-16 text-center text-[var(--es-error-500)]">
        {error}
      </div>
    );
  }
  ...
  const stats = [
    {
      ...
      sub: `${data.headcount.length} statuses`,
    ...
    },
    {
      ...
      value: String(data.todayAttendance),
      sub: "clocked in today",
      tone: "accent" as const,
    },
  ];

  // Build chart data from leavesByType
  const leaveTypeColors: Record<string, string> = {
    ANNUAL: "var(--es-accent-600)",
    SICK: "var(--es-warn-500)",
    PERSONAL: "var(--es-info-500)",
  };
      ...
      <div className="grid grid-cols-[2fr_1fr] gap-5">
        ...
        <div className="rounded-xl border border-border bg-card p-5 shadow-[var(--es-shadow-sm)]">
          <div className="mb-4 flex items-baseline justify-between">
            ...
            <div className="flex gap-3.5 text-[11px]">
              <span className="flex items-center gap-1.5">
                <span className="inline-block size-2.5 rounded-sm bg-[var(--es-accent-600)]" />
                Annual
              </span>
              <span className="flex items-center gap-1.5">
                <span className="inline-block size-2.5 rounded-sm bg-[var(--es-warn-500)]" />
                Sick
              </span>
              <span className="flex items-center gap-1.5">
                <span className="inline-block size-2.5 rounded-sm bg-[var(--es-info-500)]" />
                Personal
              </span>
            </div>
          </div>
          {leaveEntries.length === 0 ? (
            ...
            <div
              className="flex items-end gap-[18px] border-b border-border px-2.5"
              style={{ height: 200 }}
            >
                ...
                >
                  ...
                  >
                    <div
                      ...
                      style={{
                        height: (entry.count / maxCount) * 150,
        ...
        <div className="rounded-xl border border-border bg-card p-5 shadow-[var(--es-shadow-sm)]">
          ...
          {data.headcount.length === 0 ? (
            ...
            data.headcount.map((h) => {
              const pct =
                data.totalActive > 0
                  ? Math.round((h.count / data.totalActive) * 100)
                  : 0;
              return (
                <div key={h.status} className="mb-2.5">
                  ...
                  <div className="h-1.5 overflow-hidden rounded-full bg-[var(--es-neutral-100)]">
                    <div
                      className="h-full rounded-full bg-[var(--es-accent-500)]"
