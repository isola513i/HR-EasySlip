  ...
  return (
    ...
    >
      <SliderPrimitive.Control className="relative flex h-5 w-full items-center">
        <SliderPrimitive.Track className="relative h-1.5 w-full grow overflow-hidden rounded-full bg-(--es-neutral-200)">
          <SliderPrimitive.Indicator className="absolute h-full rounded-full bg-(--es-accent-600)" />
        </SliderPrimitive.Track>
        {Array.from({ length: thumbCount }).map((_, i) => (
          <SliderPrimitive.Thumb
            key={i}
            className={cn(
              "block size-4 cursor-grab rounded-full border-2 border-(--es-accent-600) bg-white shadow-(--es-shadow-sm)",
          ...
          >
            {showValueOnDrag && (
              <SliderPrimitive.Value
                className={cn(
                  "tabular-nums absolute -top-7 left-1/2 -translate-x-1/2 rounded-md bg-(--es-neutral-900) px-1.5 py-0.5 text-[11px] font-semibold text-white",
